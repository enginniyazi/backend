...
const checkCourseOwnership = (course, req) => {
        ...
        if (req.file) {
            if (course.coverImage) {
                fs.unlink(path.join(process.cwd(), course.coverImage), err => {
                    if (err)
                        console.error("Eski kurs resmi silinemedi:", err);
                });
            }
...
export const deleteCourse = async (req, res, next) => {
    try {
        ...
        if (course.coverImage) {
            fs.unlink(path.join(process.cwd(), course.coverImage), err => {
                if (err)
                    console.error("Kurs resmi silinemedi:", err);
            });
        }
...
export const addSectionToCourse = async (req, res, next) => {
    try {
        ...
        checkCourseOwnership(course, req);
        course.sections.push({ title: req.body.title });
        await course.save();
...
export const deleteSection = async (req, res, next) => {
    ...
    try {
        ...
        checkCourseOwnership(course, req);
        course.sections = course.sections.filter(sec => sec._id.toString() !== sectionId);
        await course.save();
...
export const addLectureToSection = async (req, res, next) => {
    ...
    try {
        ...
        section.lectures.push({ title, duration, videoUrl, content });
        await course.save();
...
export const updateLecture = async (req, res, next) => {
    ...
    try {
        ...
        const lecture = section.lectures.find(lec => lec._id.toString() === lectureId);
        if (!lecture) {
            res.status(404);
