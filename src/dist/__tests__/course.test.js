...
describe('Course Routes', () => {
    ...
    beforeAll(async () => {
        ...
        instructorToken = jwt.sign({ id: instructorUser._id }, JWT_SECRET, { expiresIn: '30d' });
        studentToken = jwt.sign({ id: studentUser._id }, JWT_SECRET, { expiresIn: '30d' });
    });
    afterEach(async () => {
    ...
    describe('PUT /api/courses/:id', () => {
        ...
        it('should not update other\'s course when instructor', async () => {
            ...
            const otherInstructorToken = jwt.sign({ id: otherInstructor._id }, JWT_SECRET, { expiresIn: '30d' });
            const response = await request(app)
                .put(`/api/courses/${testCourse._id}`)
